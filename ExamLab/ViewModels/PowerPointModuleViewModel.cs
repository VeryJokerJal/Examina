...
public class PowerPointModuleViewModel : ModuleViewModelBase
{
    ...
    private async Task SelectDocumentFileAsync()
    {
        ...
        {
            // 定义支持的PowerPoint文件类型
            List<string> powerPointFileTypes = [".pptx", ".ppt", ".pptm", ".ppsx"];
            ...
            {
                // 验证文件是否为有效的PowerPoint文件
                if (IsValidPowerPointFile(selectedFile))
                {
                    SelectedQuestion.DocumentFilePath = selectedFile.Path;
                    await NotificationService.ShowSuccessAsync("文件选择成功", $"已选择PowerPoint文档文件：{selectedFile.Name}");
                }
                else
                {
                    await NotificationService.ShowErrorAsync("文件类型错误", "请选择有效的PowerPoint文档文件（.pptx、.ppt、.pptm、.ppsx）");
                }
            }
        }
        catch (Exception ex)
        {
            await NotificationService.ShowErrorAsync("文件选择失败", $"选择PowerPoint文档文件时发生错误：{ex.Message}");
        }
    }
    ...
    /// <summary>
    /// 验证是否为有效的PowerPoint文档文件
    /// </summary>
    /// <param name="file">要验证的文件</param>
    /// <returns>是否为有效的PowerPoint文档文件</returns>
    private static bool IsValidPowerPointFile(Windows.Storage.StorageFile file)
    {
        if (file == null) return false;

        string extension = Path.GetExtension(file.Name).ToLowerInvariant();
        return extension == ".pptx" || extension == ".ppt" || extension == ".pptm" || extension == ".ppsx";
    }
}
