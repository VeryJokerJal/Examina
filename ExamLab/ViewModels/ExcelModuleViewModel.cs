...
public class ExcelModuleViewModel : ModuleViewModelBase
{
    ...
    /// 可用的知识点列表
    /// </summary>
    public ObservableCollection<ExcelKnowledgeConfig> AvailableKnowledgePoints { get; set; } = [];

    /// <summary>
    /// 当前选中的知识点类型
    /// </summary>
    [Reactive] public ExcelKnowledgeType? SelectedKnowledgeType { get; set; }

    /// <summary>
    /// 添加知识点操作命令
    /// </summary>
    public ReactiveCommand<ExcelKnowledgeType, Unit> AddKnowledgePointCommand { get; }
    ...
    {
        // 初始化可用知识点
        foreach (ExcelKnowledgeConfig config in ExcelKnowledgeService.Instance.GetAllKnowledgeConfigs())
        {
            AvailableKnowledgePoints.Add(config);
        }

        // 初始化命令
        AddKnowledgePointCommand = ReactiveCommand.Create<ExcelKnowledgeType>(AddKnowledgePoint);
    ...
    protected override void AddOperationPoint()
    {
        ...
        try
        {
            OperationPoint operationPoint = ExcelKnowledgeService.Instance.CreateOperationPoint(SelectedKnowledgeType.Value);
    ...
    private void AddKnowledgePoint(ExcelKnowledgeType knowledgeType)
    {
    ...
    {
        if (Enum.TryParse(knowledgeTypeString, out ExcelKnowledgeType knowledgeType))
        {
    ...
    /// 获取知识点分类
    /// </summary>
    public IEnumerable<IGrouping<string, ExcelKnowledgeConfig>> GetKnowledgePointsByCategory()
    {
    ...
    private async Task SelectDocumentFileAsync()
    {
        ...
        {
            // 定义支持的Excel文件类型
            List<string> excelFileTypes = [".xlsx", ".xls", ".xlsm", ".xlsb"];
            ...
            if (selectedFile != null)
            {
                ...
                if (IsValidExcelFile(selectedFile))
                {
                    ...
                    await NotificationService.ShowSuccessAsync("文件选择成功", $"已选择Excel文档文件：{selectedFile.Name}");
                }
                else
                {
                    await NotificationService.ShowErrorAsync("文件类型错误", "请选择有效的Excel文档文件（.xlsx、.xls、.xlsm、.xlsb）");
                }
            }
        }
        catch (Exception ex)
        {
            await NotificationService.ShowErrorAsync("文件选择失败", $"选择Excel文档文件时发生错误：{ex.Message}");
        }
    }
    ...
    /// <returns>是否为有效的Excel文档文件</returns>
    private static bool IsValidExcelFile(Windows.Storage.StorageFile file)
    {
